...
public class Day3 {
    ...
    public void sameProgramm(){

        onePathList =new ArrayList<>();
        twoPathList =new ArrayList<>();
        oneHistoryList= new ArrayList<>();
        twoHistoryList=new ArrayList<>();
        oneHistoryList.add(new Coordinate(0,0,0));
        twoHistoryList.add(new Coordinate(0,0,0));
        connections=new ArrayList<>();
        splitStringArray(oneArray,onePathList);
        splitStringArray(twoArray,twoPathList);
        makeWire(onePathList,oneHistoryList);
        makeWire(twoPathList,twoHistoryList);
        noticeConnection();
    }
    ...
    public void splitStringArray(String[] array, ArrayList<Path> list){
        for (String text:array   ) {
            String tl = text.substring(0, 1);
            int rl = Integer.parseInt(text.substring(1));
            list.add(new Path(tl,rl));
        }
    }

    public void makeWire(ArrayList<Path> pathList , ArrayList<Coordinate> history){
        int steps=0;
        for (Path path: pathList){
            steps=changeDirectionWire(path,history,steps);
        }
    }

    public int changeDirectionWire(Path path,ArrayList<Coordinate> history,int steps){
        Coordinate co = history.get(history.size()-1);
        for(int i=0;i<path.getLength();i++) {
            steps++;
            if (path.getDirection().equals("R")) {
                co.x += 1;
            } else if (path.getDirection().equals("U")) {
                co.y += 1;
            } else if (path.getDirection().equals("L")) {
                co.x -= 1;
            } else if (path.getDirection().equals("D")) {
                co.y -= 1;
            }
            history.add(new Coordinate(co.x,co.y,steps));
        }
        return steps;
    }

    public void noticeConnection(){
        for (Coordinate coordinateOne: oneHistoryList) {
            for (Coordinate coordinateTwo: twoHistoryList) {
                if(coordinateOne.x==coordinateTwo.x&&coordinateOne.y==coordinateTwo.y&&coordinateOne.x!=0&&coordinateOne.y!=0){
                    connections.add(new Coordinate(coordinateOne.x,coordinateOne.y,coordinateOne.steps+coordinateTwo.steps));
                }
            }
        }
    }
    ...
    public int shortDistance(){
        int res=-1;
        for(Coordinate cor: connections){
            int sum=Math.abs(cor.x)+Math.abs(cor.y);
            if(res==-1)
                res=sum;
            if(res>sum){
                res=sum;
